...
def teach_c(a,r,f,e,e1,e2,j):
    ...
    if (number(e1.get()) == 0 or len(e1.get())!= 3):
        ...
        return
    for i in a:
        if(i.userID != j.userID and i.emp_id == e1.get()):
...
def UGstud_c(a,r,f,e,e1,e2,e3,j):
    ...
    if (number(e1.get()) == 0 or len(e1.get())!= 5):
        ...
        return
    for i in a:
        if((i.userID !=j.userID) and i.roll_no == e1.get()):
            e1.delete(0,len(e1.get()))
            messagebox.showerror("Error","roll no already exists")
            return
    if (namec(e.get()) == 0):
...
def main(a,r):
    ...
    button2.pack()
    button3 = tk.Button(f,text = "Exit",width = 25,borderwidth=5,padx=5,pady=5,fg='red',command = r.quit)
    button3.pack()
